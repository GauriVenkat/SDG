...
<form name="marks" method="POST" onsubmit="return validateform()">
    ...
    <tr>
      <th>Unique Code</th><th>Seat Number</th>
    </tr>
  <?php
...
$p=$_SESSION['pattern'];
$ucode;$i=0;
$sql = mysqli_query($db,"select * from map where dept='$d' and sem='$s' and ccode='$p'");
...
$ucode=$row["ucode"];
 ...
 <td>      
  <input type="text" name="mks[]" value="<?php echo $row["seat"] ?>">
</td>
</tr>
...
$rowth = $_POST['mks'];
 $query = mysqli_query($db,"update map set seat=$rowth[$i] where ucode=$ucode;");
 $i++;
}
...
<input type="button" name="update" value="Update" style="width: 33%"  onclick="window.location='http://localhost/SDG-RAIT/update_st.php'"/> 
<input type="button" value="Logout" class="btn" style="width: 33%"  onclick="window.location='http://localhost/SDG-RAIT/seat.php'"/>
</form> 
</div>
</form>  
</div>
